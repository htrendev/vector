    ...
    E: FileSourceInternalEvents,
{
        ...
        S2: Future + Unpin + Send + 'static,
    {
        ...
        loop {
            ...
            // limited by the max_backoff_ms cap. Else, we set the backoff_cap to
            // its minimum on the assumption that next time through there will be
            // more lines to read promptly.
            backoff_cap = if global_bytes_read == 0 {
                cmp::min(self.max_backoff_ms, backoff_cap.saturating_mul(2))
            } else {
                1
            };
